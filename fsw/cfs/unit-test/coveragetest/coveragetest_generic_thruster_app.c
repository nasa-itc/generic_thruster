...
/*
** File: coveragetest_generic_thruster_app.c
**
** Purpose:
** Coverage Unit Test cases for the GENERIC_THRUSTER Application
**
** Notes:
** This implements various test cases to exercise all code
** paths through all functions defined in the GENERIC_THRUSTER application.
**
...
#include "ut_generic_thruster_app.h"

/* to get the GENERIC_THRUSTER_LIB_Function() declaration */
...
void Test_GENERIC_THR_AppMain(void)
{
    ...
    /*
     * GENERIC_THR_AppMain does not return a value,
     ...
     * The first is for GENERIC_THRUSTER_AppInit().  As this is in the same
     ...
     * the GENERIC_THRUSTER_AppInit() failure path was taken.
     */
    GENERIC_THR_AppMain();
     ...
     */
    UtAssert_True(GENERIC_THRUSTER_AppData.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                  "GENERIC_THRUSTER_AppData.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR",
                  (unsigned long)GENERIC_THRUSTER_AppData.RunStatus);
    ...
    UT_CheckEvent_Setup(&EventTest, GENERIC_THRUSTER_PIPE_ERR_EID, "GENERIC_THRUSTER: SB Pipe Read Error = %d");
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "GENERIC_THRUSTER_PIPE_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_GENERIC_THRUSTER_AppInit(void)
{
    /*
     * Test Case For:
    ...
    UT_TEST_FUNCTION_RC(GENERIC_THRUSTER_AppInit(), CFE_SUCCESS);
    ...
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UT_TEST_FUNCTION_RC(GENERIC_THRUSTER_AppInit(), CFE_EVS_INVALID_PARAMETER);
...
void Test_GENERIC_THRUSTER_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void GENERIC_THRUSTER_ProcessCommandPacket
     */
    ...
    UT_CheckEvent_Setup(&EventTest, GENERIC_THRUSTER_PROCESS_CMD_ERR_EID, NULL);
    ...
    FcnCode   = GENERIC_THRUSTER_NOOP_CC;
...
void Test_GENERIC_THRUSTER_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void GENERIC_THRUSTER_ProcessGroundCommand
     */
    CFE_SB_MsgId_t    TestMsgId = CFE_SB_ValueToMsgId(GENERIC_THRUSTER_CMD_MID);
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    UT_CheckEvent_Setup(&EventTest, GENERIC_THRUSTER_CMD_NOOP_INF_EID, NULL);
    GENERIC_THRUSTER_ProcessGroundCommand(Msg);
    UtAssert_True(EventTest.MatchCount == 1, "GENERIC_THRUSTER_CMD_NOOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);

    /* test dispatch of RESET */
    FcnCode = GENERIC_THRUSTER_RESET_COUNTERS_CC;
    ...
    GENERIC_THRUSTER_ProcessGroundCommand(Msg);
    UtAssert_True(EventTest.MatchCount == 1, "GENERIC_THRUSTER_CMD_RESET_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
...
void Test_GENERIC_THRUSTER_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void GENERIC_THRUSTER_ReportHousekeeping()
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(GENERIC_THRUSTER_REQ_HK_TLM);

    /* Set message id to return so GENERIC_THRUSTER_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
    ...
    UtAssert_True(MsgSend == &GENERIC_THRUSTER_AppData.HkTelemetryPkt.TlmHeader.Msg,
...
void Test_GENERIC_THRUSTER_Enable(void)
{
    ...
    UtAssert_True(EventTest.MatchCount == 1, "GENERIC_THRUSTER: Device enabled (%u)",
                  (unsigned int)EventTest.MatchCount);
...
void Test_GENERIC_THRUSTER_Disable(void)
{
    ...
    UtAssert_True(EventTest.MatchCount == 1, "GENERIC_THRUSTER: Device disabled (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    GENERIC_THRUSTER_Disable();
    UtAssert_True(EventTest.MatchCount == 1, "GENERIC_THRUSTER: UART port close error (%u)",
                  (unsigned int)EventTest.MatchCount);
...
void Test_GENERIC_THRUSTER_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool GENERIC_THRUSTER_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
    CFE_MSG_FcnCode_t fcncode = 2;
    CFE_SB_MsgId_t    msgid   = CFE_SB_ValueToMsgId(GENERIC_THRUSTER_CMD_MID);

    /*
     * test a match case
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CheckEvent_Setup(&EventTest, GENERIC_THRUSTER_LEN_ERR_EID, NULL);

    GENERIC_THRUSTER_VerifyCmdLength(NULL, size);

    /*
     * Confirm that the event was NOT generated
     */
    UtAssert_True(EventTest.MatchCount == 0, "GENERIC_THRUSTER_LEN_ERR_EID NOT generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    UT_CheckEvent_Setup(&EventTest, GENERIC_THRUSTER_LEN_ERR_EID, NULL);
    GENERIC_THRUSTER_VerifyCmdLength(NULL, size + 1);

    /*
     * Confirm that the event WAS generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "GENERIC_THRUSTER_LEN_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}
...
{
    ADD_TEST(GENERIC_THR_AppMain);
    ADD_TEST(GENERIC_THRUSTER_AppInit);
    ADD_TEST(GENERIC_THRUSTER_ProcessCommandPacket);
