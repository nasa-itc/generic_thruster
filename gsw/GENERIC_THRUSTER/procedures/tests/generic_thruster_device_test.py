import sys
import glob

for p in glob.glob('/gems/gems/openc3-cosmos-nos3-*/targets/GENERIC_THRUSTER/scripts'):
    if p not in sys.path:
        sys.path.append(p)

try:
    from openc3.script import cmd, tlm, check
    from nos3.generic_thruster_lib import *
except ImportError:
    pass

def run_generic_thruster_device_test():
    ##
    ## This script tests the cFS component device functionality.
    ## Currently this includes: 
    ##   Enable / disable, control hardware communications
    ##   Configuration, reconfigure generic_thruster instrument register
    ##


    ##
    ## Enable / disable, control hardware communications
    ##
    for n in range(GENERIC_THRUSTER_TEST_LOOP_COUNT):
        # Get to known state
        safe_generic_thruster()

        # Manually command to disable when already disabled
        cmd_cnt = tlm("GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_HK_TLM CMD_COUNT")
        cmd_err_cnt = tlm("GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_HK_TLM CMD_ERR_COUNT")
        cmd("GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_DISABLE_CC")
        get_generic_thruster_hk()
        check(f"GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_HK_TLM CMD_COUNT == {cmd_cnt}")
        check(f"GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_HK_TLM CMD_ERR_COUNT == {cmd_err_cnt+1}")

        # Enable
        enable_generic_thruster()

        get_generic_thruster_hk()

        # Confirm device counters increment without errors
        confirm_generic_thruster_data_loop()

        # Manually command to enable when already enabled
        cmd_cnt = tlm("GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_HK_TLM CMD_COUNT")
        cmd_err_cnt = tlm("GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_HK_TLM CMD_ERR_COUNT")
        cmd("GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_ENABLE_CC")
        get_generic_thruster_hk()
        check(f"GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_HK_TLM CMD_COUNT == {cmd_cnt}")
        check(f"GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_HK_TLM CMD_ERR_COUNT == {cmd_err_cnt+1}")

        # Reconfirm data remains as expected
        confirm_generic_thruster_data_loop()

        # Disable
        disable_generic_thruster()


    ##
    ##   Configuration, reconfigure generic_thruster instrument register
    ##
    for n in range(GENERIC_THRUSTER_TEST_LOOP_COUNT):
        # Get to known state
        safe_generic_thruster()

        # Confirm configuration command denied if disabled
        cmd_cnt = tlm("GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_HK_TLM CMD_COUNT")
        cmd_err_cnt = tlm("GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_HK_TLM CMD_ERR_COUNT")
        cmd("GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_PERCENTAGE_CC with THRUSTER_NUMBER 0, PERCENTAGE 0")
        get_generic_thruster_hk()
        check(f"GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_HK_TLM CMD_COUNT == {cmd_cnt}")
        check(f"GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_HK_TLM CMD_ERR_COUNT == {cmd_err_cnt+1}")
        
        # Enable
        enable_generic_thruster()

        # Set configuration
        generic_thruster_cmd(f"GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_PERCENTAGE_CC with THRUSTER_NUMBER 0, PERCENTAGE {n+1}")
        generic_thruster_cmd(f"GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_PERCENTAGE_CC with THRUSTER_NUMBER 1, PERCENTAGE {n+1}")
        generic_thruster_cmd(f"GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_PERCENTAGE_CC with THRUSTER_NUMBER 2, PERCENTAGE {n+1}")
        generic_thruster_cmd(f"GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_PERCENTAGE_CC with THRUSTER_NUMBER 3, PERCENTAGE {n+1}")

        #check(f"GENERIC_THRUSTER_DEBUG GENERIC_THRUSTER_HK_TLM DEVICE_CONFIG == {n+1}")